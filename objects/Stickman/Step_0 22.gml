...
//Is my middle center touching the floor at the start of this frame?
 var grounded = (infloor1(coll_mapid,x,bbox_bottom+1) >= 0);
//jump
...
if one_touch = 0 && two_touch = 0 && grounded = 1
{
	key_right = 0;
	key_left = 0;
}
...
p1 = tilemap_get_at_pixel(coll_mapid,bbox_side+hsp,bbox_top);
p2 = tilemap_get_at_pixel(coll_mapid,bbox_side+hsp,bbox_bottom);
...
if (tilemap_get_at_pixel(coll_mapid,x,bbox_bottom+vsp) <= 1)
{
	...
	p1 = tilemap_get_at_pixel(coll_mapid,bbox_left,bbox_side+vsp)
	p2 = tilemap_get_at_pixel(coll_mapid,bbox_right,bbox_side+vsp)
	if (p1 == 1) || (p2 == 1)
	{
...
if (grounded)
{
	...
	if ((bbox_bottom mod Tile_size) == Tile_size-1)
	{
		...
		if (tilemap_get_at_pixel(coll_mapid,x,bbox_bottom+1) > 1)
		{
			//move there
			y += abs(infloor1(coll_mapid,x,bbox_bottom+1));
		}
	}
}
