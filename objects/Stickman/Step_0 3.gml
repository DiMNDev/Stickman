...
var grounded = (infloor(coll_mapid,x,bbox_bottom+1) >= 0);
//jump
if (grounded || (infloor(coll_mapid,bbox_left,bbox_bottom+1) >= 0) || (infloor(coll_mapid,bbox_right,bbox_bottom+1) >= 0))
{
if jump = true
{
	vsp = spd_jump;
...
{
	if vsp > 0 bbox_side = bbox_bottom else bbox_vert = bbox_top;
...
var floordist = infloor(coll_mapid,x,bbox_bottom+vsp);
if (floordist >= 0)
{
	y += vsp;
	y -= (floordist + 1);
